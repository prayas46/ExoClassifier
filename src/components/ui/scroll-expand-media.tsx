  ...
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      if (!mediaFullyExpanded) {
        ...
        // Allow contracting the video when scrolling up
        e.preventDefault();
        const scrollDelta = e.deltaY * 0.0009;
        const newProgress = Math.min(
          Math.max(scrollProgress + scrollDelta, 0),
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      e.preventDefault();
      // Increase sensitivity for mobile, works in both directions
    ...
    const handleScroll = (): void => {
      // Remove the scroll lock to allow natural page scrolling
      // This enables the repeatable animation behavior
    };
